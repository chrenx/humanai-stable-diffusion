...
from utils.launch_utils5 import create_greeting, get_cuda_info, get_auth_cred, \
...
with gr.Blocks() as demo: 
    ...
    image_style.change(fn=disable_generate, outputs=[generate_button])\
                .then(handle_init_model, inputs=[user_data, image_style, image_size], 
                                           outputs=[user_data, generate_button],
    ...
    generate_button.click(fn=pre_gen_ui, 
                          inputs=[user_data]+group_ui+[loaded_model], 
                          outputs=[user_data] + group_ui + [generate_button, gen_stop_flag, loaded_model],
                          concurrency_limit=1, queue=True, trigger_mode="once") \
                    .success(fn=handle_generation,
                            inputs=[user_data, loaded_model, gen_stop_flag], 
                            outputs=[user_data, 
                                     image_output, info_output, generate_button, save_button, satisfaction, why_unsatisfied] \
                                    + group_ui + [loaded_model],
    ...
    demo.unload(lambda : print(f"<<<<<<<< USER Logout.")).then(fn=clear_cuda_mem)
...
concurrency_limit = NUM_GPU * NUM_USER_PER_GPU
app, _, _ = demo.queue(default_concurrency_limit=1).launch(
    share=True,
    auth=get_auth_cred,
    auth_message=auth_message,
    # max_threads=NUM_GPU*3+2,
    server_name="0.0.0.0",
    server_port=7860,
)
