import warnings, os, gc, time, json, csv, socket, struct, io
...
import config
from config import MODEL_NAME_PATH_MAP, CRED_FPATH, SAFETY_CHECK, REMOTE_HOST, REMOTE_PORT
...
def get_auth_cred(username, password):
    ...
    try:
        ...
        auth_socket.connect((REMOTE_HOST, REMOTE_PORT))
        auth_data = {
            "username": username,  
            "password": password  
        }
        auth_socket.send(json.dumps(auth_data).encode())
        auth_response = auth_socket.recv(1024).decode()
        auth_response = json.loads(auth_response)
        if auth_response.get('auth_status') == 'success':
            auth_socket.close()
            return True
        elif auth_response.get('auth_status') == 'failure':
            auth_socket.close()
            return False
    except Exception as e:
        MYLOGGER.info(f"Auth Error: {e}")
...
def server_image(client_socket, user_data):
    client_socket.send(json.dumps(user_data).encode()) 
    max_retries = 5
    timeout_duration = 5  # Timeout duration in seconds
    header = client_socket.recv(6)
    image_size, nsfw_detected, server_busy = struct.unpack('>I?', header)
    print(f"Expecting {image_size} bytes of data")
    print(f"Boolean value received: {nsfw_detected}")
    received_data = b''
    retries = 0
    while len(received_data) < image_size and retries < max_retries:
        try:
    ...
    if server_busy:
        MYLOGGER.info("server busy")
        return None, False, True
    if len(received_data) < image_size:
        MYLOGGER.info("Failed to receive the complete image data.")
        return None, False, False
    if nsfw_detected:
        MYLOGGER.info("nsfw_detected")
        return None, True, False
    image = Image.open(io.BytesIO(received_data))
    return image, nsfw_detected, False       
...
def handle_generate(user_data, prompt, negative_prompt, 
    ...
    try:
        ...
        gr.Warning(f"Error handling client: {e}")
        return early_return
    
    MYLOGGER.info(f"---- USER {user_data['username']} successfully connected to server")
    
    image, nsfw_detected, server_busy = server_image(client_socket, user_data)
    
    while server_busy:
        gr.Warning("Server is busy. Please wait...")
        image, nsfw_detected, server_busy = server_image(client_socket, user_data)
