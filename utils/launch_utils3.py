...
import config
from config import MODEL_NAME_PATH_MAP
...
def create_greeting(user_data, request: gr.Request):
    ...
    return user_data, welcome_msg

def update_cfg_scale(user_data, csf_scale):
    user_data['csf_scale'] = csf_scale
    return user_data, csf_scale
...
def update_negative_prompt(user_data, negative_prompt):
    user_data['negative_prompt'] = negative_prompt
    return user_data, negative_prompt

def update_prompt(user_data, prompt):
    user_data['prompt'] = prompt
    return user_data, prompt

def update_sampling_steps(user_data, sampling_steps):
    user_data['sampling_steps'] = sampling_steps
    return user_data, sampling_steps

def update_seed(user_data, seed):
    user_data['seed'] = seed
    return user_data, seed
...
def handle_generate(user_data, prompt, negative_prompt, sampling_steps, cfg_scale, seed):
    ...
    image, loaded_model = load_and_generate_image(user_data)
...
def clear_cuda_memory(loaded_model):
    MYLOGGER.info("ready to clear cuda memory")
    if loaded_model is None: return
    
    device = loaded_model.device   
    MYLOGGER.info(f"model was on device: {device}")
    ...
    MYLOGGER.info(f"GPU memory released {(after - before):.2f} MB")
...
def load_model(user_data):
    ...
    MYLOGGER.info(f"model loaded on device {best_gpu}")
    MYLOGGER.info(f"model usage of cuda: {(before - after):2f} MB")
    return loaded_model
